...
if (!type && !isadmin) {
  ...
  const token=req.headers['x-access-token'];
    ...
    try{
      ...
      console.log(decoded);
      const {rows}=await pool.query(text,[decoded.data.userId]);
      if (!rows[0]) {
        return res.status(400).send({ 'message': 'the user with this id not found' });
      }
      
      if (decoded.data.type != 'admin') {
        return res.status(401).send({status:401,'message':"You are not authorized"})
      }
    }
    catch(error){
...
try{
  ...
  return res.status(201).send({
    ...
    data:{
      token,
      id: rows[0].id,email,firstname,lastname
    }
  })
}
}
...
async login(req, res) {
    ...
    const data={rows};
      ...
      status:201,
      data:{token,id: rows[0].id,firstname: rows[0].firstname,lastname: rows[0].lastname,email:rows[0].email
    }
